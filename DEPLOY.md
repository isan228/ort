...
### 2.6 Миграция базы данных (ВАЖНО при обновлении)

**⚠️ ВНИМАНИЕ:** Если вы обновляете существующую базу данных, где модель User была изменена (телефон теперь обязателен), сначала запустите миграцию:
...
Этот скрипт:
- ✅ Проверит пользователей без телефона
- ✅ Сгенерирует временные телефоны для пользователей без телефона
- ✅ Исправит дубликаты телефонов
- ✅ Обновит структуру таблицы (phone станет обязательным и уникальным)
- ✅ Сделает email опциональным

**После миграции** запустите синхронизацию:

```bash
node sync-db.js alter
```

**Подробное руководство:** См. `MIGRATION_GUIDE.md`

### 2.7 Заполнение тестовыми данными (опционально)

```bash
node seed.js
```

### 2.8 Сборка клиентской части
